    ...
    <div className="h-screen bg-ink text-cream flex flex-col overflow-hidden">
      <header className="flex items-center justify-between px-6 py-3 border-b border-white/5 shrink-0">
        ...
        <div className="flex items-center gap-4 text-sm text-muted">
          ...
          <button
            onClick={endSession}
            className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-danger/50 text-danger hover:bg-danger/10"
          >
      ...
      <main className="flex-1 flex gap-4 p-4 min-h-0">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex-1 relative rounded-xl overflow-hidden bg-gradient-to-br from-deep to-ink flex items-center justify-center"
        >
          <AnimatedAvatar
            character="maria"
          ...
          />
          <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur px-3 py-1.5 rounded-lg">
            <span className="text-sm font-medium">{ENTREVISTADOR_AVATAR.name}</span>
          </div>
        </motion.div>
