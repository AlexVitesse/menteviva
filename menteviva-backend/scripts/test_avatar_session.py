...
Cubre:
  ...
  - provider=oss con 409 persistente (servicio lleno) -> 503 tras reintentos
  - provider=oss con 409 transitorio luego 200 -> se recupera
  - provider basura -> 503
...
class _SeqAsyncClient:
    """Stub que devuelve una SECUENCIA de status codes (uno por intento).

    El estado es de clase porque el retry crea un AsyncClient nuevo por intento.
    """
    ...
    async def post(self, url: str, json: dict | None = None):
        code = _SeqAsyncClient.statuses.pop(0) if _SeqAsyncClient.statuses else 200
        if code == 200:
            ...
            return _FakeResponse(
                200,
                {
                    ...
                    "signaling_url": f"{base}/rtc/seq-session",
                    "max_session_seconds": 1800,
                },
            )
        return _FakeResponse(code, {"detail": "MAX_SESSIONS concurrentes alcanzado"})
...
async def main() -> None:
    ...
    original_backoffs = avatar_router._CAP_BACKOFFS
    try:
        # --- provider=none -----------------------------------------------
        ...
        # --- provider=oss con 409 persistente (servicio lleno) -> 503 ----
        settings.avatar_provider = "oss"
        settings.avatar_service_url = "http://stub.local:8300"
        avatar_router._CAP_BACKOFFS = (0.0, 0.0)  # sin espera real en el test
        avatar_router.httpx = type(
            "H", (), {"AsyncClient": _SeqAsyncClient, "HTTPError": Exception}
        )
        _SeqAsyncClient.statuses = [409, 409, 409]  # 1 inicial + 2 reintentos
        try:
            await create_avatar_session(AvatarSessionRequest(avatar_id="entrevistador"))
            raise AssertionError("se esperaba HTTPException 503 (servicio lleno)")
        except HTTPException as e:
            assert e.status_code == 503, e.status_code
            assert not _SeqAsyncClient.statuses, "debio agotar los 3 intentos"
            print("OK  provider=oss 409 persistente -> 503 (tras reintentos)")

        # --- provider=oss con 409 transitorio luego 200 -> se recupera ---
        _SeqAsyncClient.statuses = [409, 200]
        resp = await create_avatar_session(AvatarSessionRequest(avatar_id="entrevistador"))
        assert resp["provider"] == "oss", resp
        assert resp["session_id"] == "seq-session", resp
        assert not _SeqAsyncClient.statuses, "debio consumir el 409 y luego el 200"
        print(f"OK  provider=oss 409 transitorio -> recupera -> {resp['session_id']}")
        ...
        avatar_router._CAP_BACKOFFS = original_backoffs
